...
export default function Address() {
  ...
  // get the current address from database
  ...
  const validate = () => {
    ...
    if (!name) {
      errorInfo = { type: "name", message: "A name is required" };
      isError = true;
    } else if (!address) {
      errorInfo = { type: "address", message: "An address is required" };
      isError = true;
    } else if (!postcode) {
      errorInfo = { type: "postcode", message: "A postcode is required" };
      isError = true;
    } else if (!city) {
      errorInfo = { type: "city", message: "A city is required" };
      isError = true;
    } else if (!country) {
      errorInfo = { type: "country", message: "A country is required" };
      isError = true;
    }
  ...
  const submit = async (event) => {
    ...
    let { isError, errorInfo } = validate();

    if (isError) {
      setError(errorInfo);
      toast.error(errorInfo.message, { autoClose: 3000 });
      return;
    }
